...
if ($state == "1") { er_pin_guard("object tracking"); }

// From /tmp: the GPIO library drops a working file in the launch directory.
exec("cd /tmp && python3 " . dirname(dirname(__DIR__)) . "/object_tracking/master.py $state");
...
if ($state == "1") {
	...
	while (time() - $began < 60) {
		...
		if (time() - $began > 3) {
			exec("pgrep -f " . escapeshellarg("$app/[o]bject_tracking/") . " > /dev/null 2>&1", $ignored, $rc);
			if ($rc !== 0) { break; }
		}
	...
	if (!$up) {
		...
		$log  = "$app/logs/object_tracking.log";
		$tail = is_file($log) ? trim((string) @shell_exec("tail -n 4 " . escapeshellarg($log))) : "";
		echo "could not start object tracking" . ($tail !== "" ? ":\n\n" . $tail : ".");
		exit;
	}
}
