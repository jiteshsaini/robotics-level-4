...
if (status=="1"):
	...
	cmd= "python3 -u " + local_path + "/" + file_name + " > " + os.path.dirname(local_path) + "/logs/object_tracking.log 2>&1 &"   # redirect: PHP waits on the inherited pipe (and /tmp is namespaced)
	print ("cmd: ", cmd)
	os.system(cmd)
	time.sleep(1) 
...
if (status=="0"):
	...
	os.system("python3 " + os.path.dirname(local_path) + "/control_panel/pwm/pwm_control.py")
